---
title: Beveiliging op rijniveau gebruiken met ingesloten inhoud in ingesloten analyses in Power BI voor betere ingesloten BI-inzichten
description: Meer informatie over de stappen die u moet uitvoeren voor het insluiten van Power BI-inhoud in uw toepassing. Maak betere geïntegreerde BI-inzichten mogelijk met geïntegreerde analytische gegevens voor Power BI.
author: KesemSharabi
ms.author: kesharab
...
4. Als u wilt controleren of de regels correct functioneren, selecteert u op het tabblad **Modellering** de optie **Als rollen weergeven**. Selecteer vervolgens de rol **Manager** die u hebt gemaakt, plus de rol **Andere gebruikers**. Voer **Andrew Ma** in als gebruiker.
...
1. Maak een Azure Analysis Services-database. Meld u vervolgens aan bij uw Azure Analysis Services-server via [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms).
...
6. Maak een PBI-rapport en publiceer dit in een werkruimte met capaciteit.
...
Klanten die on-premises SSAS-gegevensbron (SQL Server Analysis Services) met een live-verbinding configureren, profiteren van de [service-principal](embed-service-principal.md)-mogelijkheid om gebruikers en hun toegang tot gegevens in SSAS te beheren wanneer zij integreren met **Power BI Embedded**.
...
Voor het inschakelen van dit scenario gebruikt de gatewaybeheerder de [Add Datasource User Rest API](/rest/api/power-bi/gateways/adddatasourceuser) om de service-principal de *ReadOverrideEffectiveIdentity*-machtiging te verlenen voor de SSAS-gegevensbron.
...
>[!NOTE]
>Als u verbonden bent met een SSAS-database zonder dat beveiliging op rijniveau ervoor is geconfigureerd, moet u nog steeds een efficiënte identiteit (de identiteit van de SSAS-serverbeheerder) opgeven in de aanroep voor het genereren van tokens.
...
* U kunt alleen beveiliging op rijniveau gebruiken als u over een capaciteit beschikt.
