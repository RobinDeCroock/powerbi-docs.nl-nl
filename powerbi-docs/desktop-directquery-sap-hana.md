...
* **SAP HANA behandelen als een multidimensionale bron (standaard):**  In dit geval is het gedrag vergelijkbaar met dat van Power BI wanneer Power BI verbinding maakt met andere multi-dimensionale bronnen, bijvoorbeeld SAP Business Warehouse of Analysis Services. Bij het verbinden met SAP HANA met behulp van deze instelling, een één analytische of berekening weergeven is geselecteerd en alle metingen, zijn hiërarchieën en kenmerken in dat overzicht zijn beschikbaar in de lijst met velden. Terwijl er visuele elementen worden gemaakt, worden de samengevoegde gegevens altijd opgehaald uit SAP HANA. Dit is de aanbevolen aanpak en standaard voor nieuwe DirectQuery-rapporten via SAP HANA.

* **SAP HANA behandelen als een relationele bron:** In dit geval behandelt Power BI SAP HANA als een relationele bron. Dit biedt meer flexibiliteit. Wees voorzichtig met deze methode om ervoor te zorgen dat metingen worden samengevoegd zoals verwacht, en om prestatieproblemen te voorkomen.

De benadering van de verbinding wordt bepaald door een algemeen hulpmiddel, die is ingesteld door het selecteren van **bestand > Opties en instellingen** en vervolgens **opties > DirectQuery**, en de optie  **SAP HANA behandelen als een relationele bron**, zoals wordt weergegeven in de volgende afbeelding. 

![](media/desktop-directquery-sap-hana/directquery-sap-hana_01a.png)

De optie om SAP HANA als relationele bron te behandelen, bepaalt de benadering die wordt gebruikt voor *nieuwe* rapporten met behulp van DirectQuery via SAP HANA. Deze optie heeft geen effect op bestaande SAP HANA-verbindingen in het huidige rapport, noch op verbindingen in andere geopende rapporten. Dus als de optie momenteel niet is aangevinkt, wordt een nieuwe verbinding die aan SAP HANA wordt toegevoegd met behulp van **Gegevens ophalen**, gemaakt met SAP HANA als multi-dimensionale bron. Echter als een ander rapport is geopend dat ook verbinding maakt met SAP HANA, dan blijft dat rapport zich gedragen volgens de optie die was ingesteld *op het moment dat deze is gemaakt*, wat betekent dat alle rapporten die verbinding maakt met SAP HANA en die zijn gemaakte vóór februari 2018 blijven SAP HANA behandelen als een relationele bron. 

De twee benaderingen vormen verschillend gedrag en het is niet mogelijk om over te schakelen van een bestaand rapport benadering van naar het andere. 
...
* In SAP HANA kan een kenmerk worden gedefinieerd om een ander kenmerk als label te gebruiken. Bijvoorbeeld, **Product** (met waarden 1,2,3, enzovoort) kan gebruiken **ProductName** (met de waarden fiets, Shirt, handschoenen, enzovoort) als label. In dit geval wordt er één veld, **Product**, weergegeven in de lijst met velden, waarvan de waarden de labels Fiets, Shirt, Handschoenen, enzovoort zijn, maar dat wordt gesorteerd op, en met uniekheid bepaald door, de sleutelwaarden 1, 2, 3. Er wordt ook een verborgen kolom **Product.Key** gemaakt, zodat er, indien nodig, toegang tot de onderliggende sleutelwaarden kan worden verkregen. 

Eventuele variabelen die zijn gedefinieerd in de onderliggende SAP HANA-weergave, worden weergegeven op het moment van verbinden, waarna de vereiste waarden kunnen worden ingevoerd. Deze waarden kunnen vervolgens worden gewijzigd door het selecteren van **query's bewerken** op het lint en vervolgens **Parameters beheren** uit de vervolgkeuzelijst weergegeven. 
...
Als in de **gegevens ophalen** ervaring, de selecties die gemaakt zijn voor **ProductID** en de **AveragePrice** meten, dan een query over de weergave, aanvragen die is gedefinieerd Verzamel gegevens (in het eerdere voorbeeld wordt ter verduidelijking pseudo-SQL gebruikt, wat komt niet overeen met de exacte syntaxis van SAP HANA SQL). Vervolgens aggregeren eventuele verdere samenstellingen die in een visueel element worden gedefinieerd de resultaten van een dergelijke query. Dit is, net zoals hierboven beschreven voor SQL Server, opnieuw van toepassing op zowel het geval Importeren als DirectQuery. In het geval van DirectQuery, de query uit **gegevens ophalen** of **Query-Editor** wordt gebruikt in een subselectie binnen één query verzonden naar SAP HANA en het is dus niet feitelijk het geval is dat alle gegevens worden ingelezen voordat naar ze verder worden samengesteld.  
...
* In **Gegevens ophalen** of **Query-editor** mogen alleen de vereiste kolommen worden opgenomen om de vereiste gegevens op te halen. Dit weerspiegelt het feit dat het resultaat een query is en een redelijke query is die naar SAP HANA kan worden verzonden. Bijvoorbeeld als tientallen kolommen zijn geselecteerd, klikt u met de gedachte dat ze nodig zijn voor verdere visuele elementen, betekent vervolgens ook voor DirectQuery een eenvoudig visueel element dat de in de Subselectie gebruikte combinatiequery tientallen kolommen die in het algemeen wordt bevat slecht presteren.
  
We kijken naar een voorbeeld. In het volgende voorbeeld worden naast de meting *OrderQuantity* vijf kolommen geselecteerd in het dialoogvenster **Gegevens ophalen**: **CalendarQuarter**, **Color**, **LastName**, **ProductLine**, **SalesOrderNumber**. Dit betekent dat als er later een eenvoudige visual wordt gemaakt met Minus OrderQuantity, dit resulteert in de volgende SQL-query naar SAP HANA. Het gearceerde gedeelte is de subselectie. Deze bevat de query van **Gegevens ophalen** / **Query-editor**. Als de Subselectie resultaat oplevert met een hoge kardinaliteit, klikt u vervolgens erg de prestaties van SAP HANA waarschijnlijk slecht.  
...
* **Verbinding maken met SSL** : U kunt verbinding maken met behulp van importeren en multi-dimensionale met SSL, kopen kan geen verbinding met SAP HANA-instanties die zijn geconfigureerd voor het gebruik van SSL voor de relationele-connector.
