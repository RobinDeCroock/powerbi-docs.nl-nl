...
2. Typ in het bovenste vak het volledige e-mailadres voor afzonderlijke personen, distributiegroepen of beveiligingsgroepen. U kunt niet delen met dynamische distributielijsten. 
   ...
   >[!NOTE]
   >Het invoervak biedt ondersteuning voor maximaal 100 gebruikers of groepen. Als u met grote aantallen gebruikers wilt delen, kunt u het dashboard in een werkruimte maken [en het als een app distribueren](service-create-distribute-apps.md).
   > 
   > 
...
4. In het dialoogvenster **Toegang verwijderen** geeft u aan of u de toegang tot de bijbehorende inhoud, zoals rapporten en gegevenssets, ook wilt verwijderen. Als u items met een waarschuwingspictogram ![Power BI-waarschuwingspictogram](media/service-share-dashboards/power-bi-warning-icon.png) verwijdert, wordt aangeraden verwante inhoud ook te verwijderen omdat deze niet correct wordt weergegeven.
...
Ze kunnen geen inhoud van dit dashboard of rapport bewerken. Hoewel ze interactie met de grafieken kunnen hebben en filters of slicers kunnen wijzigen, kunnen ze hun wijzigingen niet opslaan. 

Alleen uw directe ontvangers kunnen het gedeelde dashboard of rapport zien. Als u bijvoorbeeld een uitnodiging stuurt naar Vicki@contoso.com, kan alleen Vicki het dashboard zien. Niemand anders kan het dashboard zien, zelfs niet als ze de koppeling hebben. Vicki moet hetzelfde e-mailadres gebruiken om het te openen. Als iemand zich aanmeldt met een ander e-mailadres, heeft hij of zij geen toegang tot het dashboard.

Personen buiten uw organisatie kunnen geen gegevens zien als rolbeveiliging of beveiliging op rijniveau is geïmplementeerd voor on-premises tabellaire modellen van Analysis Services.
...
Als u [externe gastgebruikers toestaat om inhoud in de organisatie te bewerken en te beheren](service-admin-portal.md#export-and-sharing-settings), is de standaardervaring Alleen verbruik niet op deze gebruikers van toepassing. [Meer informatie](service-admin-azure-ad-b2b.md).
...
* Iedereen met wie u uw dashboard deelt, kan dit zien en kan in de [leesweergave](consumer/end-user-reading-view.md#reading-view) interactie hebben met de gerelateerde rapporten. Ze kunnen geen rapporten maken of wijzigingen in bestaande rapporten opslaan.
...
* Collega's die uw e-maildomein delen, en collega's met een ander domein maar dat wel binnen dezelfde tenant is geregistreerd, kunnen het dashboard delen met anderen. Als bijvoorbeeld de domeinen contoso.com en contoso2.com zijn geregistreerd in dezelfde tenant en uw e-mailadres konrads@contoso.com is, kunnen zowel ravali@contoso.com als gustav@contoso2.com delen, mits u hen toestemming hebt gegeven om te delen.
...
* U kunt met maximaal 100 gebruikers of groepen delen door één keer de actie voor delen uit te voeren. U kunt echter meer dan 500 gebruikers toegang verlenen tot een item. Om dit te doen, kunt u ofwel meerdere keren delen, waarbij u de gebruikers afzonderlijk opgeeft, ofwel delen met een gebruikersgroep die alle gebruikers bevat.
...
* [Deel een gefilterd Power BI-rapport](service-share-reports.md).
